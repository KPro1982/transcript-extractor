...
const hybrid_extractor_1 = require("./hybrid-extractor");
...
async function main() {
    ...
    if (args.length === 0) {
        console.log('\n╔════════════════════════════════════════════════════════════╗');
        console.log('║          Hybrid PDF Extractor (Digital + OCR)            ║');
        console.log('║      Perfect for legal transcripts with line numbers     ║');
        console.log('╚════════════════════════════════════════════════════════════╝\n');
        console.log('Usage: npm run hybrid <pdf-file> [options]\n');
        console.log('Options:');
        console.log('  --format <txt|json|md>  Output format (default: txt)');
        console.log('  --dpi <number>          DPI for OCR (default: 300)');
        ...
        console.log('  1. Extracts main text digitally (fast, 100% accurate)');
        console.log('  2. OCRs left margin only for line numbers');
        console.log('  3. Merges both results by position\n');
        console.log('Examples:');
        console.log('  npm run hybrid "Transcripts/Deposition example.pdf"');
        console.log('  npm run hybrid document.pdf -- --format md --dpi 600\n');
        process.exit(1);
    }
    ...
    const defaultOutput = pdfPath.replace(/\.pdf$/i, `_hybrid.${format}`);
    ...
    try {
        const extractor = new hybrid_extractor_1.HybridExtractor({ dpi });
